...
(() => {
    ...
    const tileViewTemplate = require('!!raw-loader!./tile-view.html');
    /**
       * @ngdoc directive
    ...
    interface ITileViewScope extends angular.IScope {
        [key: string]: any;
        elem: any;
        tileStyle: any;
        options: any;
    ...
    APP_MODULE.directive('tdTileview', ['$compile', '$templateCache', '$timeout', '$window', ($compile : angular.ICompileService, $templateCache : angular.ITemplateCacheService, $timeout : angular.ITimeoutService, $window : angular.IWindowService) => {
        return {
            restrict: 'E',
            scope: {
                items: '=',
            ...
            link: (scope: ITileViewScope, elem : angular.IAugmentedJQuery, attrs : any) => {
                scope.elem = elem;
                scope.tileStyle = {};
                ...
                const itemContainer : angular.IAugmentedJQuery = container.children().eq(0) as angular.IAugmentedJQuery;

                let linkFunction : any;

                let heightStart = 0;
                let heightEnd = 0;

                let startRow  = 0, endRow : any;
                let renderedStartRow = -1, renderedEndRow = -1;

                let itemsPerRow: number;
                let rowCount : number;
                let cachedRowCount : number;

                let virtualRows : any[] = [];
                const scopes: IScopes = <IScopes>{};
                let scopeCounter = 0;
                ...
                function handleTileSizeChange() {
                    forEachElement((el : angular.IAugmentedJQuery) => {
                ...
                function handleTemplateUrlChange() {
                    const template = (!!scope.options.templateUrl) ? $templateCache.get<string>(scope.options.templateUrl) : scope.options.template;
                ...
                scope.$watch('options', (options : any, currentOptions : any) => {
                    // set defaults:
                ...
                var sizeDimension : string, minSizeDimension : any, orthogonalDimension : any;
                scope.$watchCollection('items', () => {
                    lastScrollPosition = Number.NEGATIVE_INFINITY;
                    layout(true);
                });

                let resizeTimeout : angular.IPromise<void>;
                ...
                function removeElement(el : angular.IAugmentedJQuery) {
                ...
                function forEachElement(fn : Function) {
                    forEachRow((row: angular.IAugmentedJQuery, rowIndex : number) => {
                ...
                function forEachRow(fn : Function) {
                ...
                function updateVisibleRows() {
                    ...
                    const rect = container[0].getBoundingClientRect() as any;
                ...
                function updateItem(elem : angular.IAugmentedJQuery, index : number, digest : any) {
                ...
                function updateRow(el : angular.IAugmentedJQuery, rowIndex : number, digest : any) {
                    const ch = el.children();
                    for (let i = 0; i < ch.length; ++i) {
                        updateItem(ch.eq(i) as angular.IAugmentedJQuery, rowIndex * itemsPerRow + i, digest);
                    }
                    ...
                    if (scope.options.alignHorizontal) {
                        if ((itemContainer as any).direction() === 'rtl') {
                            translate = -translate;
                        }
                ...
                function clearRow(row : angular.IAugmentedJQuery) {
                ...
                function removeRow() {
                    ...
                    clearRow(row as angular.IAugmentedJQuery);
                    row.remove();
                }

                function addElementToRow(row: angular.IAugmentedJQuery) {
                    const newScope = scope.$parent.$new();
                    linkFunction(newScope, function (clonedElement: angular.IAugmentedJQuery) {
                        ...
                        if (!!scope.options.padding) {
                            style = MergeObject(style, { padding: scope.options.padding + 'px' });
                        }
                ...
                function removeElementFromRow(row: angular.IAugmentedJQuery) {
                    let p = row.children().eq(-1);
                    removeElement(row.children().eq(-1) as angular.IAugmentedJQuery);
                }

                function createElements(numRows : number) {
                    updateVisibleRows();
                    ...
                    forEachRow((el : angular.IAugmentedJQuery, i : number) => {
                        virtualRows.push(el);
                ...
                function resize() {
                    ...
                    if (newComponentSize.width !== componentWidth || newComponentSize.height !== componentHeight) {
                        if (layout(false)) {
                            forEachElement((el: angular.IAugmentedJQuery) => scopes[el.attr('id')].$digest());
                        }
                    }
                }
                ...
                function layout(alwaysLayout : boolean) {
                ...
                let debounceTimeout : angular.IPromise<void>, scrollEndTimeout : angular.IPromise<void>;
